...
void Plane::handle_auto_mode(void)
{
    ...
    if (quadplane.in_vtol_auto()) {
    ...
    } else if (current_loc.alt < float(300)) { //if we are below 3m
        ...
        */
        if (nav_pitch_cd < int16_t(1000)){
            nav_pitch_cd = int16_t(1000);  //  min pitch of 10 degrees        
        } 
        ...
        calc_nav_pitch();
        calc_throttle();
        if (nav_pitch_cd > int16_t(-800)){
            nav_pitch_cd = int16_t(-800);  //  max pitch of -8 degrees
        }
    }
}
//Copy this for us! ^
