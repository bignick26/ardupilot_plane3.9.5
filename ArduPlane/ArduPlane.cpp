...
void Plane::handle_auto_mode(void)
{
    ...
    } else if (get_distance(Plane::current_loc, Plane::next_WP_loc) < 10) { //if we are within 3m of WP
        
        calc_nav_roll();
        calc_nav_pitch();
        
        if (nav_pitch_cd < int16_t(3500)){
            nav_pitch_cd = int16_t(3500);  //  min pitch of 10 degrees  add 1400 since 14 degree downpitch      
        }
